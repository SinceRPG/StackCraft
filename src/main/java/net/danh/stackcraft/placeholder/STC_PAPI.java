...
public class STC_PAPI extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player p, @NotNull String args) {
        if (args.startsWith("toggle_")) {
            ...
            return Items.getPerToggle(p, type) ? Chat.normalColorize(Files.getMessage().getString("user.status.status_on")) : Chat.normalColorize(Files.getMessage().getString("user.status.status_off"));
        } else if (args.equalsIgnoreCase("toggle")) {
            return Items.getGlobalToggle(p) ? Chat.normalColorize(Files.getMessage().getString("user.status.status_on")) : Chat.normalColorize(Files.getMessage().getString("user.status.status_off"));
        }
        return "";
    }
}
